...
parentDir               := SubStr(A_ScriptDir, 1, InStr(A_ScriptDir, "\", 0, -1) - 1)
libDir                  := parentDir "\Lib"
logDir                  := A_ScriptDIr
iconDir                 := parentDir "\Icons"
wordlistFile            := "GitHubComboList249k.txt"
wordlistPath            := libDir "\" wordlistFile
myLogFile               := "MCLog.txt"
myLogFilePath           := logDir "\" myLogFile

myAutoCorrectLibrary    := libDir "\UserHotstringsFile.ahk"
myAutoCorrectScript     := parentDir "\AutoCorrect2.ahk"
...
mclMenu.SetIcon("Log and Reload Script", parentDir "\Icons\data_backup-Brown.ico")
mclMenu.Add("Edit This Script", EditThisLog)
mclMenu.SetIcon("Edit This Script", parentDir "\Icons\edit-Brown.ico")
mclMenu.Add("Open " myLogFile, (*) => Run(myLogFilePath))
mclMenu.SetIcon("Open " myLogFile, parentDir "\Icons\TxtFile-Brown.ico")
mclMenu.Add("Analyze Manual Corrections", runAnalysis)
mclMenu.SetIcon("Analyze Manual Corrections", parentDir "\Icons\search-Brown.ico")
...
mclMenu.SetIcon("List Lines Debug", parentDir "\Icons\ListLines-Brown.ico")
mclMenu.Add("Exit Script", (*) => ExitApp())
mclMenu.SetIcon("Exit Script", parentDir "\Icons\exit-Brown.ico")
...
{
   Try
      Run(MyAhkEditorPath " " myLogFilePath)
   Catch
      Run(myLogFilePath)
}
...
tih_EndChar(tih, vk, sc)
{
   ...
   If RegExMatch(typoCache, RegEx, &out)    ; watch for pattern ..<<. ~
   {
      ...
      If (replLen > BsLen)
      {    ; replacement longer than number of BSs, so part of trigger missing.
         LastPartRepl := subStr(out.repl, '-' replLen - BsLen)
         newTrig := out.trig LastPartRepl
      }
      Else
      {    ; replacement same length as number of BSs, so entire trigger was entered.
         newTrig := out.trig
      }
      ...
      If (trigRealWord = 0) and (replRealWord = 1)
      {    ; Ensure replacement is a word, and trigger is not.
...
Appender(*)
{
   ...
   FileAppend(savedUpText, myLogFilePath)
...
runAnalysis(*)
{
   AllStrs := FileRead(myLogFilePath)    ; ahk file... Know thyself.
   oStr := "", iStr := "", Report := ""
...
CullerAppender(*)
{
   ...
   for line in StrSplit(FileRead(myLogFilePath), "`n")
   {
   ...
   myLogFileBaseName := StrSplit(myLogFilePath, ".")[1]

   If (BUchkBox.Value = 1)
      FileCopy(myLogFileBaseName '.txt', myLogFileBaseName '-BU-' A_Now '.txt', 1)
   FileDelete(myLogFilePath)    ; Delete the file so we can remake it.
   FileAppend(newFileContent, myLogFilePath)    ; Remake the file with the (now culled) string.
